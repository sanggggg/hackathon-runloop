    ...
    if (latest) {
      ...
      const verdict = !latest.finishedAt
        ...
        : t.failed
          ? red(`${t.failed} failed`)
          : green("all passed");
      cell = `${verdict} ${dim(`· ${latest.ref} · ${latest.results.length} paths`)}`;
    }
    out.push(
      `  ${pad(bold(s.id), idW)}${pad(dim(repoOf(s)), repoW)}${pad(dim(`v${s.treeVersion}`), 8)}${cell}`,
    );
  }
...
export function renderTree(suite: Suite): string {
  ...
  const out = [
    "",
    `  ${bold(suite.id)}  ${dim(repoOf(suite))}  ${dim(`tree v${suite.treeVersion}`)}`,
...
export function renderRunList(runs: Run[], suites: Map<string, Suite>): string {
  ...
  const idW = Math.max(3, ...runs.map((r) => r.id.length)) + 3;
  ...
  for (const r of runs) {
    ...
    out.push(
      `  ${pad(bold(r.id), idW)}${pad(dim(r.suiteId), suiteW)}${pad(dim(r.ref), 10)}${pad(status, 15)}${pad(dim(String(r.results.length)), 8)}${dim(humanMs(r.wallClockMs))}`,
    );
  }
