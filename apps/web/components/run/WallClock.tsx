...
/**
 * The argument for forking, kept on screen rather than in a progress view that
...
export function WallClock({ run }: { run: Run }) {
  const rows = [
    ...
    },
    {
      ...
      ms: run.wallClockMs,
      pct: (run.wallClockMs / run.sequentialEstimateMs) * 100,
      fill: "bg-kumo-brand",
    ...
    <section className="rounded-lg border border-kumo-hairline bg-kumo-base px-4 py-3">
      <div className="mb-2.5 flex items-baseline justify-between">
        ...
        <span className="text-xs text-kumo-placeholder">
          {run.results.length} paths · tree depth 3
        </span>
      </div>
      <div className="flex flex-col gap-2">
        {rows.map((r) => (
          <div key={r.label} className="flex items-center gap-3">
            ...
            <span
              className={`w-16 shrink-0 text-right text-[13px] font-semibold tabular-nums ${r.text}`}
            >
              {human(r.ms)}
            </span>
          </div>
